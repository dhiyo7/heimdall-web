...
export const parseMarkdown = (text: string): string => {
    ...
    // 3. Code: `text`
    html = html.replace(/`(.*?)`/g, '<code class="bg-[var(--color-border)] px-1 rounded font-bold text-xs">$1</code>');

    // 4. Links: [text](url)
    html = html.replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" target="_blank" rel="noopener noreferrer" class="underline text-[var(--color-primary)] hover:text-[var(--color-text-primary)]">$1</a>');
